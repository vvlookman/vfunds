                                    ...
                                    get_ticker_price(ticker, date, true, &buy_price_type).await?
                                {
                                    ...
                                    let _ = event_sender
                                        .send(BacktestEvent::Warning {
                                            title: "".to_string(),
                                            message: format!(
                                                "{buy_price_type} price of '{ticker}' not exists"
                                            ),
                                            date: Some(*date),
                                        })
                                        .await;
                            ...
                            get_ticker_price(ticker, date, true, &sell_price_type).await?
                        {
                            ...
                            let _ = event_sender
                                .send(BacktestEvent::Warning {
                                    title: "".to_string(),
                                    message: format!(
                                        "{sell_price_type} price of '{ticker}' not exists"
                                    ),
                                    date: Some(*date),
                                })
                                .await;
        ...
        for (ticker, weight) in targets_weight {
            if let Some(price) = get_ticker_price(ticker, date, true, &price_type).await? {
                ...
                let _ = event_sender
                    .send(BacktestEvent::Warning {
                        title: "".to_string(),
                        message: format!("{price_type} price of '{ticker}' not exists"),
                        date: Some(*date),
                    })
                    .await;
            }
    ...
    pub async fn position_close_with_price_type(
        ...
        if let Some(sell_price) = get_ticker_price(ticker, date, true, sell_price_type).await? {
            ...
            let _ = event_sender
                .send(BacktestEvent::Warning {
                    title: "".to_string(),
                    message: format!("{sell_price_type} price of '{ticker}' not exists"),
                    date: Some(*date),
                })
                .await;
    ...
    pub async fn position_entry_reserved_with_price_type(
        ...
        if let Some(buy_price) = get_ticker_price(ticker, date, true, buy_price_type).await? {
            ...
            let _ = event_sender
                .send(BacktestEvent::Warning {
                    title: "".to_string(),
                    message: format!("{buy_price_type} price of '{ticker}' not exists"),
                    date: Some(*date),
                })
                .await;
    ...
    tokio::spawn(async move {
        let process = async || -> VfResult<()> {
            if cv_options.cv_search {
                ...
                let count_i = search_frequencies
                    .iter()
                    .map(|v| v.frequencies.len())
                    .product::<usize>();
                let count_j = search_options
                    .iter()
                    ...
                    .enumerate()
                {
                        ...
                        .enumerate()
                    {
                        ...
                        for (k, cv_start_date) in cv_options.cv_start_dates.iter().enumerate() {
                            ...
                            match backtest_fund(&fund_definition, &options).await {
                                Ok(mut stream) => {
