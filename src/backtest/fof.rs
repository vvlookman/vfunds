    ...
    tokio::spawn(async move {
        let single_run = async |fof_definition: &FofDefinition,
            ...
            let valid_funds: Vec<_> = fof_definition
                ...
                .filter(|(_, w)| **w > 0.0)
                .collect();
            let fund_weights_sum: f64 = valid_funds.iter().map(|(_, w)| *w).sum();
            if fund_weights_sum > 0.0 {
                let workspace = { WORKSPACE.read().await.clone() };

                let mut funds_result: Vec<(usize, BacktestResult)> = vec![];
                for (fund_index, (fund_name, _)) in valid_funds.iter().enumerate() {
                ...
                let trade_dates_funds_standard_value: HashMap<NaiveDate, HashMap<usize, f64>> = {
                    let mut funds_value_map = HashMap::new();

                    for (fund_index, fund_result) in &funds_result {
                        for (date, value) in &fund_result.trade_dates_value {
                            funds_value_map
                                .entry(*date)
                                .and_modify(|v: &mut HashMap<usize, f64>| {
                                    v.insert(*fund_index, *value);
                                })
                                .or_default()
                                .insert(*fund_index, *value);
                        }
                    }
                ...
                struct PeriodStart {
                    date: NaiveDate,
                    funds_value: HashMap<usize, f64>,
                    funds_standard_value: HashMap<usize, f64>,
                }
                ...
                for date in options.start_date.iter_days().take(days as usize) {
                    if let Some(funds_standard_value) = trade_dates_funds_standard_value.get(&date)
                    {
                        if let Some(period_start) = &optional_period_start {
                            let mut funds_value: HashMap<usize, f64> = HashMap::new();
                            for (fund_index, fund_standard_value) in funds_standard_value {
                                if let Some(period_start_fund_value) =
                                    period_start.funds_value.get(fund_index)
                                    && let Some(period_start_fund_standard_value) =
                                        period_start.funds_standard_value.get(fund_index)
                                {
                                    let fund_value = period_start_fund_value * fund_standard_value
                                        / period_start_fund_standard_value;
                                    funds_value.insert(*fund_index, fund_value);
                                }
                            }

                            // Check frequency
                            let days = (date - period_start.date).num_days();
                            let period_days = fof_definition.frequency.to_days();
                            if period_days > 0 && days >= period_days as i64 {
                                // Rebalance
                                let total_value = funds_value.values().sum::<f64>();
                                let mut new_funds_value: HashMap<usize, f64> = HashMap::new();
                                for (fund_index, (_, fund_weight)) in valid_funds.iter().enumerate()
                                {
                                    let target_fund_value =
                                        *fund_weight / fund_weights_sum * total_value;

                                    let fund_value = if let Some(fund_value) =
                                        funds_value.get(&fund_index)
                                    {
                                    ...
                                    new_funds_value.insert(fund_index, fund_value);
                                }

                                trade_dates_value
                                    .push((date, new_funds_value.values().sum::<f64>()));
                            ...
                            // Init
                            let mut funds_value: HashMap<usize, f64> = HashMap::new();
                            let mut period_start_funds_standard_value: HashMap<usize, f64> =
                                HashMap::new();
                            for (fund_index, fund_standard_value) in funds_standard_value {
                                if let Some((_, fund_weight)) = valid_funds.get(*fund_index) {
                                    funds_value.insert(
                                        *fund_index,
                                        *fund_weight / fund_weights_sum * fund_standard_value,
                                    );
                                    period_start_funds_standard_value
                                        .insert(*fund_index, *fund_standard_value);
                                }
                            }
