...
impl RuleExecutor for Executor {
    async fn exec(
        ...
        let ps_quantile_lower = self
            .options
            .read_f64_in_range("ps_quantile_lower", 0.0, 0.0..=1.0);
        ...
        let watch_period_days = self.options.read_u64_no_zero("watch_period_days", 30);
                        ...
                        if let (
                            ...
                            if *pe > pe_overvalued || *ps > ps_overvalued {
                                ...
                                if *pe > pe_sell || *ps > ps_sell {
                                    ...
                                    rule_send_info(
                                        rule_name,
                                        &format!("[Overvalued] {ticker_title} PE:{pe:.2}>{pe_overvalued:.2}~{pe_sell:.2} || PS:{ps:.2}>{ps_overvalued:.2}~{ps_sell:.2}"),
                                        date,
                                        event_sender,
                                        )
                                    .await;
                        ...
                        if let (
                            ...
                            if *pe < pe_undervalued && *ps < ps_undervalued {
                                ...
                                if *pe < pe_buy && *ps < ps_buy {
                                    ...
                                    rule_send_info(
                                        rule_name,
                                        &format!("[Undervalued] {ticker}({ticker_title}) PE:{pe:.2}<{pe_undervalued:.2}~{pe_buy:.2} && PS:{ps:.2}<{ps_undervalued:.2}~{ps_buy:.2}"),
                                        date,
                                        event_sender,
                                    )
                                    .await;
